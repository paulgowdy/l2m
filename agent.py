...
from env_wrapper import * #PendulumWrapper, LunarLanderContinuousWrapper, BipedalWalkerWrapper

class Agent:

    def __init__(self, sess, env, seed, n_agent=0, vis = False, mode = '3D'):
        ...
        self.env_wrapper = L2MWrapper(env, vis, mode)
    ...
    def write_train_logs(self):
        ...
        #with open('train_logs/state_collection_agent_' + str(self.n_agent) + '.txt', 'w') as f:

        #    f.write(str(self.state_collect))

        with open('train_logs/agent_' + str(self.n_agent) + '.txt', 'a') as f:

            for i in range(self.print_freq):

                f.write(str(self.train_rewards[i - self.print_freq]) + ',' + str(self.train_durs[i - self.print_freq]) + ',' + str(self.train_steps[i - self.print_freq]) + '\n')
    ...
    def run(self, PER_memory, gaussian_noise, run_agent_event, stop_agent_event):
        ...
        while not stop_agent_event.is_set():
            ...
            while not ep_done:
                ...
                #print(str(self.n_agent) + ', ' + str(num_steps))
                ## Take action and store experience
                if train_params.RENDER:
                    self.env_wrapper.render()

                #self.state_collect.append(state)
                ...
                if terminal or num_steps == train_params.MAX_EP_LENGTH:
                    ...
                    ep_dur = ep_end_time - ep_start_time

                    #print(self.n_agent, ep_dur, num_steps)

                    self.train_rewards.append(episode_reward)
                    self.train_durs.append(ep_dur)
                    self.train_steps.append(num_steps)

                    #print(ep_rewards)

                    if len(self.train_rewards) % self.print_freq == 0:

                        print('Agent ' + str(self.n_agent) + ', Episode ' + str(len(self.train_rewards)) + ': ' + str(episode_reward) + ', ' + str(float(ep_dur/num_steps)))

                        self.write_train_logs()
