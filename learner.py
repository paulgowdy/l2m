...
from network import Actor, Actor_BN, Critic, Critic_BN
...
class Learner:
    ...
    def build_network(self):
        ...
        # pretraining op
        #self.pretrain_pred_action_ph = tf.placeholder(tf.float32, ((16,) + train_params.ACTION_DIMS))
        #self.obs_pretrain_ph = tf.placeholder(tf.float32, ((16,) + train_params.STATE_DIMS))
        '''
        these are the lines for pretraining

        self.pretrain_target_action_ph = tf.placeholder(tf.float32, ((train_params.BATCH_SIZE,) + train_params.ACTION_DIMS))

        self.actor_pretrain_step = self.actor_net.pretrain_step(self.pretrain_target_action_ph, self.state_ph)
        '''
    ...
    '''
    def pretrain(self, obs, actions):

        # obs come in as np arrays
        # individual obs are lists
        ...
        for e in range(200):
            ...
            for i in range(30):
                ...
                obs_minibatch = obs[i*minibatch_size:(i+1)*minibatch_size,]
                action_minibatch = actions[i*minibatch_size:(i+1)*minibatch_size,]

                #print(obs_minibatch.shape)
                #print(action_minibatch.shape)

                #predicted_actions = self.sess.run(self.actor_net.output, {self.state_ph:obs_minibatch})

                #pretrain_loss, _ = self.sess.run([], {self.pretrain_pred_action_ph: predicted_actions, self.pretrain_target_action_ph:action_minibatch})

                pretrain_loss, _ = self.sess.run([self.actor_net.pretrain_loss, self.actor_pretrain_step], {self.state_ph:obs_minibatch, self.pretrain_target_action_ph:action_minibatch})

                #pretrain_loss, _ = self.sess.run([self.actor_net.pretrain_loss, self.actor_pretrain_step], {self.obs_pretrain_ph:obs_minibatch, self.target_action_pretrain_ph:action_minibatch})

                #print(e, i, pretrain_loss)
                z.append(pretrain_loss)

        plt.figure()
        plt.plot(z)
        plt.show()

        self.saver.save(self.sess, self.checkpoint_path, global_step=i)
        sys.stdout.write('\nCheckpoint saved.\n')
        sys.stdout.flush()
    '''
    def run(self):
        ...
        for train_step in range(self.start_step+1, train_params.NUM_STEPS_TRAIN+1):
            ...
            # This is what prints the steps line
            ...
            if train_step % 500 == 0:
                ...
                print('Learner', str(train_step), now - train_start_time)
