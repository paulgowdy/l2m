...
import numpy as np
from ops import dense, relu, tanh, batchnorm, softmax
...
class Actor:
    def __init__(self, state, state_dims, action_dims, action_bound_low, action_bound_high, dense1_size, dense2_size, final_layer_init, scope='actor'):
        ...
        with tf.variable_scope(self.scope):
            ...
            self.bn_params = [] # No batch norm params
    '''
    def pretrain_step(self, pretrain_target_action_ph, pretrain_pred_action_ph):

        with tf.variable_scope(self.scope):


            self.pretrain_optimizer = tf.train.AdamOptimizer(0.001)

            #pretrain_actions = self.output

            self.pretrain_loss = tf.losses.mean_squared_error(pretrain_target_action_ph, pretrain_pred_action_ph)

            pretrain_step = self.pretrain_optimizer.minimize(self.pretrain_loss, var_list=self.network_params)

            return pretrain_step
    '''

    def pretrain_step(self, pretrain_target_action_ph, pretrain_obs_ph):

        with tf.variable_scope(self.scope):

            self.pretrain_optimizer = tf.train.AdamOptimizer(0.001)

            pretrain_pred_action = self.output

            self.pretrain_loss = tf.losses.mean_squared_error(pretrain_target_action_ph, pretrain_pred_action)
